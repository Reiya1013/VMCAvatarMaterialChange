...
//--------------------------------------------------------------------------------------------------------------------------
// BeatSaber—p
    void BeatSaber_Alpha(inout lilFragData fd)
    {
        bool isAudioLink = false;
#if defined(LIL_FEATURE_AUDIOLINK)
        isAudioLink = _UseAudioLink;
#endif

        fd.col.a = 0.0f;
        if (isAudioLink)
        {
            float a = fd.emissionColor * 10; //float((fd.audioLinkValue / 2) + 
            fd.col.a = a;//clamp(a, 0, 1);
        }
        else
        {
            float a = (fd.emissionColor / 50); //float((fd.audioLinkValue / 2) + 
            fd.col.a += saturate(a);//clamp(a, 0, 1);
        }
    }
