...
float4 frag(VertexOutput i) : COLOR {
    ...
    float3 finalColor = emissive + finalcolor2;
        ...
        float emissiveA = emissive.rgb;
    #else
        float emissiveA = 1;// emissive.rgb;
    #endif

    #ifdef ARKTOON_FADE
        fixed _AlphaMask_var = UNITY_SAMPLE_TEX2D_SAMPLER(_AlphaMask, REF_MAINTEX, TRANSFORM_TEX(i.uv0, _AlphaMask)).r;
        #ifdef ARKTOON_REFRACTED
            fixed4 finalRGBA = fixed4(lerp(sceneColor, finalColor, (_MainTex_var.a*REF_COLOR.a*_AlphaMask_var*chAlpha)),emissiveA); // K2Ex:chAlphaを追加
        #else
            fixed4 finalRGBA = fixed4(finalColor,(_MainTex_var.a*REF_COLOR.a*_AlphaMask_var*chAlpha*emissiveA)); // K2Ex:chAlphaを追加
        #endif
    #else
        fixed4 finalRGBA = fixed4(finalColor,emissiveA);
    #endif
