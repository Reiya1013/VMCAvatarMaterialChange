...
// 本Shaderは上記のsynqark様制作のデータを、MIT Licenceに基づき、改造・二次配布しているものです。改造者：空々こん

Shader "BeatSaber/arktoon/_K2Extra/3ChFadeRefracted" {
    Properties {
        // Double Sided
        ...
        _ChMaskTex ("[Common] ChMask Texture", 2D) = "white" {}
        _ColorC1 ("[Common] 1Ch Color", Color) = (1,1,1,1)
        _ColorC2 ("[Common] 2Ch Color", Color) = (1,1,1,1)
        _ColorC3 ("[Common] 3Ch Color", Color) = (1,1,1,1)
        ...
        _RefractionFresnelExpCh1 ("[Refraction] Ch1 Fresnel Exp",  Range(0, 10)) = 0
        _RefractionFresnelExpCh2 ("[Refraction] Ch2 Fresnel Exp",  Range(0, 10)) = 0
        _RefractionFresnelExpCh3 ("[Refraction] Ch3 Fresnel Exp",  Range(0, 10)) = 0
        _RefractionStrengthCh1 ("[Refraction] Ch1 Strength",  Range(-2, 2)) = 0
        _RefractionStrengthCh2 ("[Refraction] Ch2 Strength",  Range(-2, 2)) = 0
        _RefractionStrengthCh3 ("[Refraction] Ch3 Strength",  Range(-2, 2)) = 0
        ...
        _GlossBlendCh1 ("[Gloss] Ch1 Smoothness", Range(0, 1)) = 0.5    // K2Ex
        _GlossBlendCh2 ("[Gloss] Ch2 Smoothness", Range(0, 1)) = 0.5    // K2Ex
        _GlossBlendCh3 ("[Gloss] Ch3 Smoothness", Range(0, 1)) = 0.5    // K2Ex
        _GlossPowerCh1 ("[Gloss] Ch1 Metallic", Range(0, 1)) = 0.5      // K2Ex
        _GlossPowerCh2 ("[Gloss] Ch2 Metallic", Range(0, 1)) = 0.5      // K2Ex
        _GlossPowerCh3 ("[Gloss] Ch3 Metallic", Range(0, 1)) = 0.5      // K2Ex
        _GlossColorCh1 ("[Gloss] Ch1 Color", Color) = (1,1,1,1)         // K2Ex
        _GlossColorCh2 ("[Gloss] Ch2 Color", Color) = (1,1,1,1)         // K2Ex
        _GlossColorCh3 ("[Gloss] Ch3 Color", Color) = (1,1,1,1)         // K2Ex
        // Outline
        ...
        _MatcapTexture1ch ("[MatCap] 1Ch Texture", 2D) = "black" {} // K2Ex
        _MatcapColor1ch ("[MatCap] Color", Color) = (1,1,1,1)       // K2Ex
        _MatcapTexture2ch ("[MatCap] 2Ch Texture", 2D) = "black" {} // K2Ex
        _MatcapColor2ch ("[MatCap] Color", Color) = (1,1,1,1)       // K2Ex
        _MatcapTexture3ch ("[MatCap] 3Ch Texture", 2D) = "black" {} // K2Ex
        _MatcapColor3ch ("[MatCap] Color", Color) = (1,1,1,1)       // K2Ex
        // Reflection
        ...
        _ReflectionReflectionPower1ch ("[Reflection] 1ch Reflection Power", Range(0, 1)) = 1    // K2Ex
        _ReflectionReflectionPower2ch ("[Reflection] 2ch Reflection Power", Range(0, 1)) = 1    // K2Ex
        _ReflectionReflectionPower3ch ("[Reflection] 3ch Reflection Power", Range(0, 1)) = 1    // K2Ex
        ...
        _ShadowcapTexture1ch ("[ShadowCap] 1Ch Texture", 2D) = "white" {} // K2Ex
        _ShadowColor1ch ("[ShadowCap] Color", Color) = (1,1,1,1)       // K2Ex
        _ShadowcapTexture2ch ("[ShadowCap] 2Ch Texture", 2D) = "white" {} // K2Ex
        _ShadowColor2ch ("[ShadowCap] Color", Color) = (1,1,1,1)       // K2Ex
        _ShadowcapTexture3ch ("[ShadowCap] 3Ch Texture", 2D) = "white" {} // K2Ex
        _ShadowColor3ch ("[ShadowCap] Color", Color) = (1,1,1,1)       // K2Ex
        // vertex color blend
    ...
    SubShader {
        ...
        Pass {
            ...
            #include "cginc/K2Ex_arkludeFrag3Ch.cginc"
            ENDCG
        }
        Pass {
            Name "FORWARD_DELTA"
            ...
            #include "cginc/arkludeOther.cginc"
            #include "cginc/arkludeVertGeom.cginc"
            #include "cginc/K2Ex_arkludeAdd3Ch.cginc"
            ENDCG
        }
    ...
    FallBack "Standard"
    CustomEditor "ArktoonShaders.K2EX_ArktoonInspector"
}
